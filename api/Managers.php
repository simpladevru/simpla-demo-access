...
{	
	...
	private static $current_manager;
	...
	public function get_managers()
	{
		...
		foreach($lines as $line)
		{
			if(!empty($line))
			{
				$manager = null;
				$fields = explode(":", $line);
				$manager = new \Root\api\models\Manager();
	...
	public function get_manager($login = null)
	{
        ...
        if( !empty(static::$current_manager)) {
		    return static::$current_manager;
        }

		foreach($this->get_managers() as $manager) {
			if($manager->login == $login) {
				return static::$current_manager = $manager;
            }
		}		
		return false;	
	}
